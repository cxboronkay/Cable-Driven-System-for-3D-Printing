...
class GCoord:
    ...
    def __init__(self, x, y, z):
        ...
        self.y = round(y,10)
        self.z = round(z, 10)
        # self.e = round(e, 10)           ##4th coordinate previously included (such as for extrusion)
...
with open('gcode_circle_offset2.txt') as f:            
    allLines = f.readlines()
f.close()

# create list to store Gcoordinates in
Gcoord_list = []
Gcoord_list.append(GCoord(0,0,0))
...
for line in allLines:
    ...
    if len(lineItems) != 0:
        ...
        if lineItems[0]== 'G1':
            ...
            Gcoord_list.append(GCoord(xcoord,ycoord,zcoord))
...
def convert_coord(gcoord):                      ##calculations made based on G-code written for Prusa Mini (180x180mm print bed)
    xc = GCoord.getX(gcoord)/180*400                ## and on pulley configuration with 400mm between pulleys (see schematic)
    yc = GCoord.getY(gcoord)/180*400
    zc = GCoord.getZ(gcoord)/180*400
    # ec = GCoord.getE(gcoord)/180*399
    A = round(math.sqrt(xc**2 + yc**2 + (zc-250)**2))               ## rounds to integers
    B = round(math.sqrt((xc-400)**2 + yc**2 + (zc-250)**2))
